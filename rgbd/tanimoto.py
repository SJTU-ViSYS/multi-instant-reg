...
LastEditTime: 2022-08-06 14:22:50
...
def refine_labels(data, labels, iou_mask_thresh=0.8, min_num4solve_model=3, with_preference=True, max_distance=0.0625, iter_num=3, num_thresh_multipler=5, init_inlier_mask=None, device='cuda'):
    '''
    input torch [n,c]
          torch [n]
    return torch [n]
    ...
    for iter_i in range(iter_num):
        ...
        num_thresh = min(min_num4solve_model*(num_thresh_multipler**iter_i), int(data_num/100))
        model_labels = output[counts >= num_thresh]
        print(f'refine iter: {iter_i} counts = {counts[counts >= num_thresh]} num_thresh={num_thresh}')
...
def me(output_labels: torch.Tensor, gt_labels: torch.Tensor):
    ...
    for i, iou_matrix in enumerate(batch_iou_matrix):
        ...
        print(f'label_corr = {true_ind}, {pred_ind}')
    ...
    for bi in range(b):
        ...
        for i in range(max(true_labels[bi])+1):
            ...
            if true_mask.sum():
                ...
                label_sum += 1
                print(i, pred_mask.sum(),true_mask.sum(), (true_mask & pred_mask).sum()/(true_mask | pred_mask).sum())
        mean_iou += (iou/label_sum)/b

    return batched_mean_miss_rate, assigned_labels, mean_iou

def pred_model(data, pred_labels, min_num4solve_model=3, device='cuda'):
    '''
    torch [n,c]
            [n]
    retrun: bool, [m,16]
    ...
    hs_mask = torch.arange(idx.shape[0]).to(idx)[diff_over_the_biggest<0.1] #0.25
