...
def random_rotation(points, normals=None, max_degree=360, max_amp=3, minamp=0, bil=False):
    # inputs:[N, 3], None/[N, 3], [N, 3]
    x, degree, amp = np.random.rand(6), np.random.rand(1)*max_degree*np.pi/180, np.random.rand(1)*max_amp + minamp
    if bil:
        amp = np.random.rand(1)*2*max_amp - max_amp
    w, v= x[:3], x[3:]
