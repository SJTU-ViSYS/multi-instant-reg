...
from datasets.scan2cadDatasets_box import Scan2CAD_oneTomore_box

from datasets.multiRot_multi import MultiRotDataset_multi
...
def collate_fn_descriptor(list_data, config, neighborhood_limits):
    ...
    #, bboies
    for ind, (src_pcd,tgt_pcd,src_feats,tgt_feats,rot,trans,matching_inds, src_pcd_raw, tgt_pcd_raw, sample, bboies) in enumerate(list_data):
    ...
    for block_i, block in enumerate(config.architecture):
        #if batched_points.shape[0]<20 and batched_points.shape[0]:
        #print(f'batched_points.shape = {batched_points.shape},batched_lengths={batched_lengths}')
        ...
        if 'pool' in block or 'strided' in block:
            ...
            #print(f'pool_b = {pool_b}')
            #print(f'pool_p.shape = {pool_p.shape}')
...
def calibrate_neighbors(dataset, config, collate_fn, keep_ratio=0.8, samples_threshold=500):
    timer = Timer()
...
def get_datasets(config):
    if(config.dataset=='multi_multi'):
        train_set = MultiRotDataset_multi(config, inlier_thresh=0.25, corr_num=256, split='train', with_normal=False)
        val_set = MultiRotDataset_multi(config, inlier_thresh=0.25, corr_num=256, split='valid', with_normal=False)
        benchmark_set = MultiRotDataset_multi(config, inlier_thresh=0.25, corr_num=256, split=config.benchmark_set_split, with_normal=False)
    elif(config.dataset=='box'):
        ...
        train_set = Scan2CAD_oneTomore_box(config, 'train', shapenetroot, scannetroot, scan2cadroot, 1024, ds_vs=0.05, with_scale=False, points_thresh=30000)
        val_set = Scan2CAD_oneTomore_box(config, 'valid', shapenetroot, scannetroot, scan2cadroot, 1024, ds_vs=0.05, with_scale=False, points_thresh=30000)
        benchmark_set = Scan2CAD_oneTomore_box(config, 'test', shapenetroot, scannetroot, scan2cadroot, 1024, ds_vs=0.05, with_scale=False, points_thresh=30000)
